...
| **疑似投流** | 赞/千播低于分档红线(常规 <8、10w+ <7、50w+ <4)**且**评论/千播 <0.5;或 ≥3w 且评论 ≤5 | 与自然内容分开观察 |

**赞率和评论密度同时偏低才标疑似投流**,次周播放基本不动(页面标「播放停滞」)作辅助参考。单看赞率会误判:
懒人包、长访谈这类内容赞率天然偏低,但评论和次周增长都正常(2026-09-14 据此更正了 Finance730 的误标)。
播放越大互动浓度越容易下降,所以赞率红线随量级调整。所有「投流」都是公开数据推断,无法外部核实。
...
- `data/` — 每周原始数据存档
- `curation/` — 每周运营拆解(人写):`brief` 供 leader 30 秒扫读,`actions` 供周会分工,其余字段承载证据与长文
- `weeks/` — 每期归档页(本地多页版专用,云端版不用这个,靠 build_artifact.py 内嵌历史)
- `thumb_cache/` — 缩略图下载缓存,按文件名去重,不会重复下载同一张图

## 已验证的行业打法(读数据前先知道这些,免得把常规操作误读成信号)

**访谈节目是「一访多期」,不是「同一个人做了两次」。** 嘉宾到访一次,制作方评估素材里有多少
可挖的点,再剪成 2 期甚至更多:通常一期偏理念与人生经历,一期偏当下观点与策略。判断依据是简介
里的拍摄日期——光大《名人說》郭思治 EP19/EP20 都注明「拍摄日期 2026 年 8 月 4 号」,雷鼎鸣
EP17/EP18 同理,富途的陆叔也是同样做法。**所以两期之间的数据差异是「同一批素材、不同切法」的
对照,不是「改进后重做」。**

这个对照恰恰给出最有用的结论——**按嘉宾的稀缺性分配选题**:

| 嘉宾 | 偏人生/本地那期 | 偏行情/美股那期 |
|---|---|---|
| 郭思治(光大 EP19/20) | 26.16w(半世纪人生、人格、独门心法) | 4.28w(恒指观点) |
| 雷鼎鸣(光大 EP17/18) | 3.24w(经济学家为什么不炒楼、香港楼市) | 2.18w(英伟达、台积电、美股降息) |

港股背景的老前辈讲美股本来就没什么可讲,硬讲反而暴露短板;而半世纪的行业人生、性格、方法论
是别人复制不了的。**讲人家不擅长的,数据上很容易扑街。**

更大的背景:访谈类内容的竞争已经不在行情解读上了,各家都在往差异化走——人物故事、演绎、
更自然的场景(郭思治那期就带了健身、灵异、电影顾问花絮)。看这类内容时要把它当人物内容评估,
不要只看它讲了什么观点。
