...
## 自动更新(已配置好,不用管)
每周只会真跑一次,但有**三个触发时机**,哪个先到算哪个:

1. **周一 09:05** — 电脑醒着且在线时的正常路径
2. **开机/登录时** — 周一没开机、周三才开也能补跑
3. **一连上网就跑** — 合盖带出门、到公司连上 WiFi 的瞬间会触发

靠 `logs/.last_done` 里的「本周已完成」戳保证不重复:戳还在就 0.4 秒静默退出,
戳在每周一零点自然失效,所以新一周的第一次触发就会真跑。采集期间有并发锁,
网络抖动重复触发也不会跑出两份。
...
cat ~/finance-hits/logs/.last_done     # 看本周跑没跑过
launchctl list | grep finance-hits     # 第二列是上次退出码,0=成功
bash ~/finance-hits/weekly.sh          # 手动跑(本周跑过的话会直接跳过)
rm ~/finance-hits/logs/.last_done      # 想强制重跑就删掉戳
```
